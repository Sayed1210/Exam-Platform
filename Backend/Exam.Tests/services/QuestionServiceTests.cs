...
namespace Exam.Tests.Services
{
    public class QuestionServiceTests
    {
        ...
        public async Task CreateAsync_Should_Create_Question_With_Choices()
        {
            ...
            _repoMock.Setup(r => r.CreateAsync(It.IsAny<Question>())).ReturnsAsync(created);
            _repoMock.Setup(r => r.GetByIdAsync(1)).ReturnsAsync(created);

            var result = await _service.CreateAsync(MakeCreateRequest());
        ...
        [Fact]
        public async Task CreateAsync_Should_Trim_Text()
        {
            var created = MakeQuestion(1);
            ...
            var request = new QuestionRequest
            {
                TopicId = 1,
                Text = "  What is 2+2?  ",
                Choices = [new ChoiceRequest { Text = "4", IsCorrect = true }]
            };

            await _service.CreateAsync(request);

            _repoMock.Verify(r => r.CreateAsync(
                It.Is<Question>(q => q.Text == "What is 2+2?")), Times.Once);
        }
        ...
        public async Task UpdateAsync_Should_Update_Text_Only()
        {
            ...
            var result = await _service.UpdateAsync(1,
                new UpdateQuestionRequest { Text = "Updated text?" });
        ...
        [Fact]
        public async Task UpdateAsync_Should_Update_Choices_When_Provided()
        {
            var existing = MakeQuestion(1);
            var updated = new Question
            {
            ...
            var result = await _service.UpdateAsync(1, new UpdateQuestionRequest
            {
                Choices =
                [
            ...
            result.Should().NotBeNull();
            result!.Text.Should().Be(existing.Text);
        ...
        public async Task UpdateChoiceAsync_Should_Update_Specific_Choice()
        {
            ...
            var result = await _service.UpdateChoiceAsync(1, 1,
                new ChoiceRequest { Text = "Updated A", IsCorrect = false });
        ...
        public async Task UpdateChoiceAsync_Should_Return_Null_When_Question_NotFound()
        {
            ...
            var result = await _service.UpdateChoiceAsync(1, 1,
                new ChoiceRequest { Text = "X", IsCorrect = true });
        ...
        public async Task UpdateChoiceAsync_Should_Return_Null_When_Choice_NotFound()
        {
            ...
            var result = await _service.UpdateChoiceAsync(1, 99,
                new ChoiceRequest { Text = "X", IsCorrect = true });
        ...
        public async Task GetPagedAsync_Should_Return_Correct_Page()
        {
            _repoMock.Setup(r => r.GetPagedAsync(1, 2))
                     .ReturnsAsync(([MakeQuestion(1), MakeQuestion(2)], 10));
        ...
        [Fact]
        public async Task GetPagedAsync_Should_Calculate_TotalPages_With_Ceiling()
        {
            _repoMock.Setup(r => r.GetPagedAsync(1, 3))
                     .ReturnsAsync((new List<Question>(), 10));
