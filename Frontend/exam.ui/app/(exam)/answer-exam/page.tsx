...
//       if (result.message === "Exam time expired") {
...
//   // get saved answers from backend
//   useEffect(() => {
//     if (!exam) return;
...
export default function ExamPage() {
  ...
  useEffect(() => {
    ...
    console.log("exam from session:", parsed);
    setExam(JSON.parse(storedExam));
  }, []);
  ...
  // timer — seeded from backend's remainingSeconds
  useEffect(() => {
    if (!exam) return;
    ...
    const interval = setInterval(() => {
      setTimeLeft((prev) => {
        ...
        if (prev <= 1) {
          ...
          submitExam(exam.examId, exam.candidateId, formattedAnswers).then((result) => {
            if (!result.success && result.message !== "Exam time expired") {
              toast.error(result.message);
            } else {
      ...
      <div className="flex-1 h-screen p-8 flex flex-col overflow-y-auto">
        ...
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-muted">Question {current + 1} of {TOTAL_QUESTIONS}</p>
          </div>

          {/* Timer Box */}
          <div
            className={`w-42 h-18 flex items-center justify-center rounded-xl border text-title transition-colors shadow-sm
              ${timeLeft <= 300
                ? "bg-red-100 border-red-200 text-primary"
                : "bg-green-100 border-green-200 text-black"
              }
        ...
        <div className="mb-6">
          ...
          <p className="text-muted mt-2">{answeredCount} of {TOTAL_QUESTIONS} answered</p>
        </div>
