...
namespace Exam.Tests.Services
{
    public class ExamServiceTests
    {
        ...
        {
            _repoMock.Setup(r => r.AddAsync(It.IsAny<ExamEntity>()))
                     .Returns(Task.CompletedTask);
            _repoMock.Setup(r => r.GetWithQuestionsAndChoicesAsync(It.IsAny<int>()))
                     .ReturnsAsync(MakeEmptyExam(1));

            var result = await _service.CreateExamAsync(new CreateExamRequest
            {
                Title = "Algo",
                DurationMins = 50,
                QuestionIds = []
        ...
        public async Task UpdateExamAsync_Should_Update_Title_Only()
        {
            ...
            var result = await _service.UpdateExamAsync(1,
                new UpdateExamRequest { Title = "New Title" });
        ...
        // ── DELETE ────────────────────────────────────────────────────
        [Fact]
        public async Task DeleteExamAsync_Should_Return_True()
        {
        ...
        [Fact]
        public async Task AssignQuestionsAsync_Should_Skip_Already_Assigned()
        {
            var exam = new ExamEntity
            {
                Id = 1,
                ExamQuestions = [new() { ExamId = 1, QuestionId = 2 }]
            };
        ...
        public async Task RemoveQuestionAsync_Should_Remove_Question()
        {
            ...
            _repoMock.Setup(r => r.GetWithQuestionsAndChoicesAsync(1))
                     .ReturnsAsync(new ExamEntity { Id = 1, ExamQuestions = [] });
        ...
        [Fact]
        public async Task RemoveQuestionAsync_Should_Return_Null_When_Exam_NotFound()
        {
            _repoMock.Setup(r => r.GetByIdAsync(1)).ReturnsAsync((ExamEntity?)null);
